...
class Game_Board
        ...
        def initialize(outer_board = nil, putahi = nil)
                if !outer_board
                        @outer_board = Array.new
                        8.times do 
                                @outer_board << Piece.new(
                                        @outer_board.length > 3 ? "B" : "A"
                                )   
                        end
                else
        ...
        def to_s()
                (0..6).inject([]) do |array, index|
                        ...
                        when index%2 == 0
                                array.last[2] = get_string_value(
                                        @outer_board[index/2]
                                )
                                array.last[12] = get_string_value(
                                        @outer_board[-1-index/2]
                                )
                        end
                        array
                end
        ...
        def lost?(cp)
                ...
                @outer_board.each_with_index do |piece, spot|
